	...
	<div style="width: 100%; height: calc(100%);">
		<div id="one">
			<i class="icon icon-align-left"
				style="margin-right: 10px; margin-left: 10px;"></i>长江经济带小水电数量排名
		</div>
		...
		<div id="one">
			<i class="icon icon-align-left"
				style="margin-right: 10px; margin-left: 10px;"></i>长江经济带小水电装机规模排名
		</div>
		<table class="table" align="center">
			<thead class="fixedThead" align="center">
				<tr>
...
<script>
$(function(){
	countDz_right("i-two",${param.bj});
	countZjzgm_right("i-twor",${param.bj});
});
function countZjzgm_right(css,bj){
	if(bj == null){
		bj = 0;
	}
	if(css == "i-oner"){
	...
	$.ajax({
		url:basePath+"homePage/homePage!countZjzgm_right_cj.action",
		data:{"css":css,"bj":bj},
		type: "POST",
		dataType:"json",
...
function countDz_right(css,bj){
	...
	$.ajax({
		url:basePath+"homePage/homePage!countDz_right_cj.action",
		data:{"css":css,"bj":bj},
		type: "POST",
		dataType:"json",
		success:function(response){
