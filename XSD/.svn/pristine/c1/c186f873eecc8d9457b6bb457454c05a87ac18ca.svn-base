(function($) {
	$.zhengcefagui = function(option) {
		//option自定义参数覆盖
		//A、界面控件变量
		var opt_control={
			 btn_add		:$('#btn_add_user')		 //增加按钮
			,btn_del     	:$('#btn_del_user')		 //删除按钮	
			,btn_ref     	:$('#btn_ref_zcfg')		 //刷新按钮
			,table 			:$('#tbinfo_user')		 //BootStrapTable的ID
			,searchInput	:$('#searchName_zcfg')	 //模糊查询input
		...
		//C、请求地址URL
		var opt_url={
			 url_list		:basePath+"policy/zcfg!list.action"		//查询数据URL
			,url_flag		:basePath+"policy/zcfg!flag.action"		//审核数据URL
		...
		function queryParams(params) {  // 配置参数
		    ...
		    var temp = {   // 这里的键的名字和控制器的变量名必须一直，这边改动，控制器也需要改成一样的
		       "mZcfgInfoFormBean.pageBean.limit"		: params.limit   // 页面大小
		      ,"mZcfgInfoFormBean.pageBean.offset"		: params.offset  // 当前记录偏移条数
		      ,"mZcfgInfoFormBean.pageBean.sort"		: params.sort  // 排序列名
		      ,"mZcfgInfoFormBean.pageBean.sortOrder"	: params.order// 排位命令（desc，asc）
		      ,"mZcfgInfoFormBean.searchName"				: opt_control.searchInput.val() // 查询关键字
		    };
		...
		this.showPDF = function(aaa){
			window.open("periodicTable_show.jsp");  
		}
		...
		//初始化审核框
		this._flag_=function(fgbm){
			_flag(fgbm);
		}
		function _flag(fgbm){
			var obj={
				"mZcfgInfoFormBean.zcfgInfo.fgbm":fgbm
			};
			$.ajax({
				data:obj,
