...
/**
 * 通过最大，最小值自动计算X,Y轴的刻度值
 * @param cormax
 * @param cormin
 * @param cornumber
 * @returns {Array} cormax 最大值,cormin 最小值,cornumber 刻度数,tmpstep 间隔数值
 */
function standard(cormax,cormin,cornumber){
	var dard=getStandard(cormax,cormin,cornumber);
	if(dard!=null && dard[2]>cornumber){
		dard=getStandard(dard[0],dard[1],cornumber);
	}
	return dard;
}
function getStandard(cormax,cormin,cornumber){
	cormax=parseFloat(cormax);
	cormin=parseFloat(cormin);
	var tmpmax,tmpmin, corstep,tmpstep,tmpnumber,temp,extranumber;
	if(parseFloat(cormax)<=parseFloat(cormin))
		return ;
...
//	选取规范步长
	if(tmpstep>=0&&tmpstep<=0.01){
		...
		tmpstep = 0.02;
	}else if(tmpstep>=0.020001&&tmpstep<=0.03){
		tmpstep = 0.03;
	}else if(tmpstep>=0.030001&&tmpstep<=0.04){
		tmpstep = 0.04;
	}else if(tmpstep>=0.040001&&tmpstep<=0.05){
		tmpstep = 0.05;
	}else if(tmpstep>=0.050001&&tmpstep<=0.06){
		tmpstep = 0.06;
	}else if(tmpstep>=0.060001&&tmpstep<=0.07){
		tmpstep = 0.07;
	}else if(tmpstep>=0.070001&&tmpstep<=0.08){
		tmpstep = 0.08;
	}else if(tmpstep>=0.080001&&tmpstep<=0.1){
		tmpstep = 0.1;
	}else if(tmpstep>=0.100001&&tmpstep<=0.2){
		tmpstep = 0.2;
	}else if(tmpstep>=0.200001&&tmpstep<=0.25){
		tmpstep = 0.25;
	}else if(tmpstep>=0.250001&&tmpstep<=0.3){
		tmpstep = 0.3;
	...
	cornumber = tmpnumber;
	cormax=cornumber*tmpstep;
	return [(parseFloat(Number(cormax).toFixed(2))+parseFloat(Number(cormin).toFixed(2))),parseFloat(Number(cormin).toFixed(2)),cornumber,tmpstep];
}
/**
 * 加载div通用函数
 * @param divId
