...
function initHeight(){
	var screenHeight=window.screen.availHeight-430;
...
function xiugai(){
	 layer.open({
  		  ...
  		  icon: 6,
  		  content: basePath+'business/zhengcefagui/add.jsp?fgbm=',
  	});
}
function serch(){
...
function initTable(fgly,ssbm,startDate,endDate,searchName){
  layui.use('table', function(){
   	  ...
   	  table.render({
   	    ...
   	    ,cols: [[
   	      {toolbar: '#id',field:'fgbm', title:'序号', width:80, fixed: 'left', unresize: true}
   	      ,{field:'bt', title:'法规内容', width:350, fixed: 'left',align:'left'}
   	      ,{field:'ly', title:'法规来源', width:150, }
   	      ,{field:'ssfl', title:'法规分类', width:150, }
   	      ,{field:'zdjg', title:'发布单位', width:200,  sort: true}
   	      ,{field:'wh', title:'首次发布文号', width:250}
   	      ,{field:'pbrq', title:'发布/最后修订日期',width:150}
	      ,{field:'ssrq', title:'实施日期',width:150}
