...
#mainonedes_qg_jszt  thead th {
	padding: 3px 0px;
	white-space: normal;
}

#mainonedes_qg_jszt  .layui-table[lay-size=sm] td, .layui-table[lay-size=sm] th
	{
	padding: 3px 0px;
	white-space: nowrap;
}

#mainonedes_qg_kffs  thead th {
	padding: 3px 0px;
	white-space: normal;
}

#mainonedes_qg_kffs  .layui-table[lay-size=sm] td, .layui-table[lay-size=sm] th
	{
	padding: 3px 0px;
	white-space: nowrap;
}
		...
		<div class="container-fluid">
			<div class="row">
				<div class="col-md-6" style="margin-top: 2px;">
					<ul class="list-group list-group-one" style="position: relative">
						<li class="list-group-item" id="item-one"><i
							class="icon icon-area-chart" style="margin-right: 5px"></i>全国小水电建设状态统计
							<button id="table-icon" class="btn btn-primary table-icon"
				...
				<div class="col-md-6" style="margin-top: -8px;">
					<ul class="list-group list-group-one" style="position: relative">
						<li class="list-group-item" id="item-one"><i
							class="icon icon-area-chart" style="margin-right: 10px"></i>全国小水电开发方式统计
							<button id="table-icon_kffs" class="btn btn-primary table-icon"
								type="button">切换表格</button>
							<button style="margin-right: 5px;" id="kffs-iconone"
								class="btn btn-primary table-icon desplay" type="button"
								onclick="export_jstl_qg(2)">导出表格</button></li>
						<li class="list-group-item" id="maintwo"
							style="height: 300px; border-bottom-left-radius: 5px; border-bottom-right-radius: 5px;">
						</li>
						<li class="list-group-item desplay" id="mainonedes_qg_kffs"
							style="padding: 0px; position: absolute; left: 0px; top: 32px; width: 100%; z-index: 333; height: 300px;">
							<div class="layui-table-header">
								<table class="layui-table classtableone" lay-size="sm"
									style="margin: 0px;">
				...
				<div class="col-md-12">
					<ul class="list-group list-group-one" style="position: relative">
						<li class="list-group-item" id="item-one"><i
							class="icon icon-area-chart" style="margin-right: 10px"></i>全国小水电发展历程
							<button id="table-icon_qg_dz_fzlc"
								class="btn btn-primary table-icon" type="button">切换表格</button>
							<button style="margin-right: 5px;" id="fzlc-iconone"
								class="btn btn-primary table-icon desplay" type="button"
								onclick="export_jstl_qg(3)">导出表格</button></li>
						<li class="list-group-item" id="mainthree"
							style="height: 300px; border-bottom-left-radius: 5px; border-bottom-right-radius: 5px;">
						</li>
						<li class="list-group-item desplay" id="mainonedes_qg_kffs_date"
							style="padding: 0px; position: absolute; left: 0px; top: 32px; width: 100%; z-index: 333; height: 300px;">
							<div class="layui-table-header">
								<table class="layui-table classtableone" lay-size="sm"
									style="margin: 0px;">
							...
							<table class="layui-table classtablethird" lay-size="sm"
								...
								<tbody id="zj_table_qg_kffs_date">
								</tbody>
							</table>
						</li>
					</ul>
...
function export_jstl_qg(num){
	 ...
	 var fileName;
	 if(num==1){
		 fileName = "全国小水电建设状态统计" 
		  tabTitle = ['序号','地区','总装机数量（万千瓦）','总电站数量（座）','拟建装机规模（万千瓦）','拟建电站数量（座）'
			 ,'在建装机规模（万千瓦）','在建电站数量（座）','运行装机规模（万千瓦）','运行电站数量（座）','废弃装机规模（万千瓦）'
			 ,'废弃电站数量（座）','拆除装机规模（万千瓦）','拆除电站数量（座）','未知建设状态装机规模（万千瓦）','未知建设状电站数量（座）']
		  ...
		  sql = " select name, "
			      ...
			      +" (SELECT ISNULL(Round(SUM(zjrl)/10000,2), 0) from hps_info_sum1 where sheng=A.name and jszt=6) as rl6, "
			      +" (SELECT count(id) from hps_info_sum1 where sheng=A.name and jszt=6) as dz6 "
			      ...
			      +" (SELECT ISNULL(Round(SUM(zjrl)/10000,2), 0) from hps_info_sum1 where jszt=6) as rl6,  "
			      +" (SELECT count(id) from hps_info_sum1 where jszt=6) as dz6 "
			      +" FROM hps_info_sum1 "
	 }else if(num==2){
		 fileName = "全国小水电开发方式统计" 
...
function initTable(bj){
	$.ajax({
		...
		success:function(response){
			...
			if(bj == "qg_jszt"){
				$("#zj_table_qg_jszt").html("");
				$("#zj_table_qg_jszt").append(zj_tr);
			}
			if(bj == "qg_kffs"){
...
function initKffs(bj){
    ...
    if(bj == null){
     ...
     //点击切换图标文字和显示隐藏table
     $('#table-icon_kffs').click(function(){
    	 if( $('#mainonedes_qg_kffs').hasClass('desplay')){
    		 $('#table-icon_kffs').text('切换图表');
    		 $('#mainonedes_qg_kffs').removeClass('desplay');
    		 ...
    		 initTable("qg_kffs")
    		  ...
    		  $('#mainonedes_qg_kffs').addClass('desplay');
    		  $('#table-icon_kffs').text('切换表格');
...
function initJszt(bj){
 		...
 		success:function(response){
				     ...
				     var option = {
				     		 ...
				     		 tooltip: {
				     		    ...
				     		    legend: {
				     		        ...
				     		        bottom: 5,
				     		        data:leng,
				     		    },//['#007bbb','#008e57','#933d92','#c48f00','#A0522D'],
			...
			myChart.on('click', function (params) {
				...
				layer.open({
					  ...
					  skin: 'layui-layer-molv' ,
					  icon: 6,
					  content: basePath+'business/cezhanchaxun/modalFrame.jsp?name='+name+"&bj="+0,
					});
				});
				     // 使用刚指定的配置项和数据显示图表。
				     myChart.setOption(option);
     ...
     $('#table-icon').click(function(){
    	 if( $('#mainonedes_qg_jszt').hasClass('desplay')){
    		 $('#table-icon').text('切换图表');
    		 $('#mainonedes_qg_jszt').removeClass('desplay');
    		 ...
    		 initTable("qg_jszt")
    		  ...
    		  $('#mainonedes_qg_jszt').addClass('desplay');
    		  $('#table-icon').text('切换表格');
    		  $('#table-iconone').addClass('desplay');
    	 }
     })
}
...
function initZjrl(bj){
    ...
    //点击切换图标文字和显示隐藏table
    $('#table-icon_qg_zj_fzlc').click(function(){
   	 if( $('#mainonedes_qg_kffs_date_zj').hasClass('desplay')){
   		 $('#table-icon_qg_zj_fzlc').text('切换图表');
   		 $('#mainonedes_qg_kffs_date_zj').removeClass('desplay');
		 if($('#qg_zj_fzlc-iconone').hasClass('desplay')){
			 $('#qg_zj_fzlc-iconone').removeClass('desplay');
		 }else{
			 $('#table-iconone').addClass('desplay');
		 }
   		 initTable("qg_kffs_date_zj")
   	 }
   	  else{
   		  $('#mainonedes_qg_kffs_date_zj').addClass('desplay');
   		  $('#table-icon_qg_zj_fzlc').text('切换表格');
...
function initDzsl(bj){
    ...
    //点击切换图标文字和显示隐藏table
    $('#table-icon_qg_dz_fzlc').click(function(){
   	 if( $('#mainonedes_qg_kffs_date').hasClass('desplay')){
   		 $('#table-icon_qg_dz_fzlc').text('切换图表');
   		 $('#mainonedes_qg_kffs_date').removeClass('desplay');
   		 if($('#fzlc-iconone').hasClass('desplay')){
			 $('#fzlc-iconone').removeClass('desplay');
		 }else{
			 $('#table-iconone').addClass('desplay');
		 }
   		 initTable("qg_kffs_date")
   	 }
   	  else{
   		  $('#mainonedes_qg_kffs_date').addClass('desplay');
   		  $('#table-icon_qg_dz_fzlc').text('切换表格');
