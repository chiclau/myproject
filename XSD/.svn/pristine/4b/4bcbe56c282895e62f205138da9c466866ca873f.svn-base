...
function HashMap() {
    this.length = 0;
    // this.prefix = "hashmap_prefix_20170101_";
    this.prefix = "";
    this.data={};
}
/**
...
HashMap.prototype.get = function(key) {
    return typeof(this.data[this.prefix + key]) == "undefined" ? null : this.data[this.prefix + key];
}
/**
...
HashMap.prototype.values = function() {
    ...
    for(var strKey in this.data) {
        if(strKey.substring(0,this.prefix.length) == this.prefix)
            arrValues[index ++] = this.data[strKey];
    }
...
HashMap.prototype.putAll = function(map) {
    ...
    for(var i in arrKey)
    	this.data.put(arrKey[i],arrValue[i]);
}
//toString
HashMap.prototype.toString = function() {
    ...
    for(var strKey in this.data) {
        if(strKey.substring(0,this.prefix.length) == this.prefix)
            str += strKey.substring(this.prefix.length) + " : " + this.data[strKey] + "\r\n";
    }
    return str;
}