...
public class EnvironmentDao extends HibernateBaseDao<NyjHpsInfo> {
	...
	public List<Map> echartsAndTable(String tableId, String arr) {
		...
		if (tableId.equals("sthj_id")) {
			...
			sql.append(" SUM(CASE WHEN H.stllxfcs is null  then H.zjrl else 0 end )AS zrl3, ");
			sql.append(" SUM(CASE WHEN H.stllxfcs is null  then 1 else 0 end )AS zdz3 ");
			sql.append(" from hps_info_sum1 H LEFT JOIN Area A  ON A.id = H.sd_sheng_id OR A.id = H.sd_shi_id  WHERE H.jszt in(1,2) ");
		...
		if (tableId.equals("stll_id")) {
			...
			sql.append(" SUM(CASE WHEN H.stlljkss is null then H.zjrl else 0 end )AS zrl3, ");
			sql.append(" SUM(CASE WHEN H.stlljkss is null then 1 else 0 end )AS zdz3 ");
			sql.append(" from hps_info_sum1 H LEFT JOIN Area A  ON A.id = H.sd_sheng_id OR A.id = H.sd_shi_id WHERE H.jszt in(1,2)   ");
			sql.append(" GROUP BY A.name,A.id HAVING COUNT(H.id) > 0  ");
			sql.append(" AND A.id IN (" + arr + ") ");
			sql.append(" ORDER BY A.id ");
		...
		if (tableId.equals("gycs_id")) {
			...
			sql.append(" SUM(CASE WHEN H.gycs is null   then H.zjrl else 0 end )AS zrl3, ");
			sql.append(" SUM(CASE WHEN H.gycs is null   then 1 else 0 end )AS zdz3 ");
			sql.append(" from hps_info_sum1 H LEFT JOIN Area A  ON A.id = H.sd_sheng_id OR A.id = H.sd_shi_id  WHERE H.jszt in(1,2) ");
			sql.append(" GROUP BY A.name,A.id HAVING COUNT(H.id) > 0   ");
			sql.append(" AND A.id IN (" + arr + ") ");
			sql.append(" ORDER BY A.id ");
		...
		if (tableId.equals("zzfl_id")) {
			...
			sql.append(" SUM(CASE WHEN H.zzflcs is null   then H.zjrl else 0 end )AS zrl3, ");
			sql.append(" SUM(CASE WHEN H.zzflcs is null   then 1 else 0 end )AS zdz3 ");
			sql.append(" from hps_info_sum1 H LEFT JOIN Area A  ON A.id = H.sd_sheng_id OR A.id = H.sd_shi_id  WHERE H.jszt in(1,2) ");
			sql.append(" GROUP BY A.name,A.id HAVING COUNT(H.id) > 0   ");
			sql.append(" AND A.id IN (" + arr + ") ");
			sql.append(" ORDER BY A.id ");
		...
		if (tableId.equals("gh_id")) {
			...
			sql.append(" SUM(CASE WHEN H.sffhgh  is null    then H.zjrl else 0 end )AS zrl3, ");
			sql.append(" SUM(CASE WHEN H.sffhgh  is null    then 1 else 0 end )AS zdz3 ");
			sql.append(" from hps_info_sum1 H LEFT JOIN Area A  ON A.id = H.sd_sheng_id OR A.id = H.sd_shi_id  WHERE H.jszt in(1,2) ");
			sql.append(" GROUP BY A.name,A.id HAVING COUNT(H.id) > 0  ");
			sql.append(" AND A.id IN (" + arr + ") ");
			sql.append(" ORDER BY A.id ");
		}
		if (tableId.equals("hp_id")) {//是否开展项目环评
			sql.append(" select A.name,SUM(H.zjrl) AS zrl,COUNT(H.id) AS zdz, ");
			...
			sql.append(" SUM(CASE WHEN H.sffhghhp  is null    then H.zjrl else 0 end )AS zrl3, ");
			sql.append(" SUM(CASE WHEN H.sffhghhp  is null    then 1 else 0 end )AS zdz3 ");
			sql.append(" from hps_info_sum1 H LEFT JOIN Area A  ON A.id = H.sd_sheng_id OR A.id = H.sd_shi_id  WHERE H.jszt in(1,2) ");
			sql.append(" GROUP BY A.name,A.id HAVING COUNT(H.id) > 0  ");
			sql.append(" AND A.id IN (" + arr + ") ");
			sql.append(" ORDER BY A.id ");
		}
		if (tableId.equals("hp_id1")) {//是否有环评审批
			sql.append(" select A.name,SUM(H.zjrl) AS zrl,COUNT(H.id) AS zdz, ");
			sql.append(" SUM(CASE WHEN H.sffhghhp = '是' then H.zjrl else 0 end )AS zrl1, ");
			sql.append(" SUM(CASE WHEN H.sffhghhp = '是' then 1 else 0 end )AS zdz1, ");
			sql.append(" SUM(CASE WHEN H.sffhghhp = '否' then H.zjrl else 0 end )AS zrl2, ");
		...
		if (tableId.equals("ghhp_id")) {//规划环评
			sql.append(" select A.name,SUM(H.zjrl) AS zrl,COUNT(H.id) AS zdz, ");
			sql.append(" SUM(CASE WHEN H.sffhghhp = '是' then H.zjrl else 0 end )AS zrl1, ");
			sql.append(" SUM(CASE WHEN H.sffhghhp = '是' then 1 else 0 end )AS zdz1, ");
			sql.append(" SUM(CASE WHEN H.sffhghhp = '否' then H.zjrl else 0 end )AS zrl2, ");
		...
		if (tableId.equals("jgys_id")) {//过鱼设施
			sql.append(" select A.name,SUM(H.zjrl) AS zrl,COUNT(H.id) AS zdz, ");
			...
			sql.append(" SUM(CASE WHEN H.sftgjghbys  is null    then H.zjrl else 0 end )AS zrl3, ");
			sql.append(" SUM(CASE WHEN H.sftgjghbys  is null    then 1 else 0 end )AS zdz3 ");
			sql.append(" from hps_info_sum1 H LEFT JOIN Area A  ON A.id = H.sd_sheng_id OR A.id = H.sd_shi_id WHERE H.jszt in(1,2)  ");
			sql.append(" GROUP BY A.name,A.id HAVING COUNT(H.id) > 0  ");
			sql.append(" AND A.id IN (" + arr + ") ");
			sql.append(" ORDER BY A.id ");
		}
		if (tableId.equals("bxtsgh_id")) {//坝下
			sql.append(" select A.name,SUM(H.zjrl) AS zrl,COUNT(H.id) AS zdz, ");
			...
			sql.append(" SUM(CASE WHEN H.bxsfcztsgk  is null    then H.zjrl else 0 end )AS zrl3, ");
			sql.append(" SUM(CASE WHEN H.bxsfcztsgk  is null    then 1 else 0 end )AS zdz3 ");
			sql.append(" from hps_info_sum1 H LEFT JOIN Area A  ON A.id = H.sd_sheng_id OR A.id = H.sd_shi_id WHERE H.jszt in(1,2)  ");
			sql.append(" GROUP BY A.name,A.id HAVING COUNT(H.id) > 0  ");
			sql.append(" AND A.id IN (" + arr + ") ");
			sql.append(" ORDER BY A.id ");
		}
		if (tableId.equals("hpqk_id")) {//环评情况
			...
			sql.append(" SUM(CASE WHEN H.sftgxmhp='否'   then H.zjrl else 0 end )AS zrl2, ");
			sql.append(" SUM(CASE WHEN H.sftgxmhp='否'     then 1 else 0 end )AS zdz2, ");
			sql.append(" SUM(CASE WHEN H.sftgxmhp is null   then H.zjrl else 0 end )AS zrl3, ");
			sql.append(" SUM(CASE WHEN H.sftgxmhp is null   then 1 else 0 end )AS  zdz3  ");
			sql.append(" from date_sum D  LEFT JOIN  hps_info_sum1 H  ON DATENAME(yy, H.dysj) = D.date AND H.sd_shi_id IN (" + arr + ")  AND  H.jszt in(1,2) ");
		...
		if (tableId.equals("tcqhpqk_id")) {
			...
			sql.append(" SUM(CASE WHEN H.hpspsj IS NULL then H.zjrl else 0 end )AS zrl3, ");
			sql.append(" SUM(CASE WHEN H.hpspsj IS NULL  then 1 else 0 end )AS zdz3 ");
			sql.append(" from date_sum D  LEFT JOIN  hps_info_sum1 H  ON DATENAME(yy, H.dysj) = D.date  AND H.sd_shi_id IN (" + arr + ") ");
			sql.append(" GROUP BY D.date HAVING D.date IS NOT NULL AND D.date <= GETDATE() ");
			sql.append(" ORDER BY D.date DESC ");
		}
		if (tableId.equals("sjzrbhqk_id")) {//涉及自然保护区情况
			sql.append(" select A.name,SUM(H.zjrl) AS zrl,COUNT(H.id) AS zdz, ");
			sql.append(" SUM(CASE WHEN H.sfsjzrbhq = '是' AND H.jszt in(1,2) then H.zjrl else 0 end )AS zrl1, ");
			sql.append(" SUM(CASE WHEN H.sfsjzrbhq = '是' AND H.jszt in(1,2) then 1 else 0 end )AS zdz1, ");
			sql.append(" SUM(CASE WHEN H.sfsjzrbhq = '否' AND H.jszt in(1,2) then H.zjrl else 0 end )AS zrl2, ");
			sql.append(" SUM(CASE WHEN H.sfsjzrbhq = '否' AND H.jszt in(1,2) then 1 else 0 end )AS zdz2, ");
			sql.append(" SUM(CASE WHEN H.hxq = '涉及' or wfq='涉及' and  H.sfsjzrbhq = '是' AND H.jszt in(1,2) then H.zjrl else 0 end )AS zrl3, ");
			sql.append(" SUM(CASE WHEN H.hxq = '涉及' or wfq='涉及' and  H.sfsjzrbhq = '是' AND H.jszt in(1,2) then 1 else 0 end )AS zdz3, ");
			sql.append(" SUM(CASE WHEN H.hcq = '涉及' and  H.sfsjzrbhq = '是' AND H.jszt in(1,2) then H.zjrl else 0 end )AS zrl4, ");
			sql.append(" SUM(CASE WHEN H.hcq = '涉及' and  H.sfsjzrbhq = '是'  AND H.jszt in(1,2) then 1 else 0 end )AS zdz4, ");
			sql.append(" SUM(CASE WHEN H.sys = '涉及'  and  H.sfsjzrbhq = '是' AND H.jszt in(1,2) then H.zjrl else 0 end )AS zrl5, ");
			sql.append(" SUM(CASE WHEN H.sys = '涉及'  and  H.sfsjzrbhq = '是' AND H.jszt in(1,2)  then 1 else 0 end )AS zdz5 ,");
			sql.append(" SUM(CASE WHEN H.sfsjzrbhq is null AND H.jszt in(1,2) then 1 else 0 end )AS zdz6, ");
			sql.append("  SUM(CASE WHEN H.sfsjzrbhq is null  AND H.jszt in(1,2) then H.zjrl else 0 end )AS zrl6 ");
			sql.append(" from hps_info_sum1  H LEFT JOIN Area A  ON A.id = H.sd_sheng_id OR A.id = H.sd_shi_id  WHERE H.jszt in(1,2)  ");
			sql.append(" GROUP BY A.name,A.id HAVING COUNT(H.id) > 0  ");
			sql.append(" AND A.id IN (" + arr + ") ");
			sql.append(" ORDER BY A.id ");
	...
	public List<Map> sjzrbhqqk(String xzqhdm) {
		...
		sql.append(" from hps_info_sum1 WHERE sfsjzrbhq='是' AND jszt in(1,2)  ");
		if (!xzqhdm.equals("") && xzqhdm != null) {
			String xians = xzqhdm.replace(",", "' , '");
			if (xians != null && !xians.equals("")) {
		...
		sql.append("  AND sheng IN ("+cjjjd+") ");
		sql.append(" ) as b  unpivot (value FOR name IN([试验区],[缓冲区],[核心区（未分区）])) t");
		return this.createSQLQuerybyMap(sql.toString());
	}

	public List<Map> sjzrbhqqk_1(String sheng,String shi,String xian) {
		StringBuffer sql = new StringBuffer("SELECT name,value FROM (select  ");
		sql.append(" SUM(case WHEN sys='涉及' then 1 else 0 end )as '试验区', ");
		sql.append(" SUM(case WHEN hcq='涉及' then 1 else 0 end )as '缓冲区' ,");
		sql.append(" SUM(case WHEN hxq='涉及' or wfq='涉及' then 1 else 0 end )as '核心区（未分区）' ");
	...
	public List<Map> queryHuanBao3(String xzqhdm) {
		...
		sql.append("FROM hps_info_sum1 WHERE 1=1 AND jszt in(1,2)  ");
		if (!xzqhdm.equals("") && xzqhdm != null) {
			String xians = xzqhdm.replace(",", "' , '");
			if (xians != null && !xians.equals("")) {
				sql.append("  AND  xian  IN (' " + xians + "  '  )");
	...
	public List<Map> queryHuanBao3_1(String sheng,String shi,String xian) {
		StringBuffer sql = new StringBuffer();
		sql.append("SELECT  (CASE sfsjzrbhq WHEN '是' THEN '是' WHEN '否' THEN '否' ELSE '未知' END) as name,");
		sql.append("COUNT(CASE sfsjzrbhq WHEN '是' THEN '是' WHEN '否' THEN '否' ELSE '未知' END) AS value ");
		sql.append("FROM hps_info_sum1 WHERE 1=1 AND jszt in(1,2)  ");
	...
	public List<Map> kzxmhpqkS_1(String sheng,String shi,String xian) {
		StringBuffer sql = new StringBuffer();
		sql.append("select  SUM(case WHEN dysj<'1986' then 1 else 0 end )as 'y1',");
		sql.append(" SUM(case WHEN dysj BETWEEN '1986' AND '1989' then 1 else 0 end )as 'y2',");
		sql.append(" SUM(case WHEN dysj BETWEEN '1989' AND '1998' then 1 else 0 end )as 'y3',");
	...
	public List<Map> tcqWanCheng_1(String sheng,String shi,String xian) {
		StringBuffer sql = new StringBuffer();
		sql.append("select  SUM(case WHEN dysj<'1986' then 1 else 0 end )as 'y1',");
		sql.append(" SUM(case WHEN dysj BETWEEN '1986' AND '1989' then 1 else 0 end )as 'y2',");
		sql.append(" SUM(case WHEN dysj BETWEEN '1989' AND '1998' then 1 else 0 end )as 'y3',");
	...
	public List<Map> llxfTotal(String xzqhdm) {
		...
		sql.append("from hps_info_sum1  where 1=1 AND jszt in(1,2) ");//只统计在建，运行
		if (!xzqhdm.equals("") && xzqhdm != null) {
			String xians = xzqhdm.replace(",", "' , '");
			if (xians != null && !xians.equals("")) {
				sql.append("  AND  xian  IN (' " + xians + "  '  )");
	...
	public List<Map> llxfTotal_1(String sheng,String shi,String xian) {
		StringBuffer sql = new StringBuffer();
		sql.append("SELECT name,value FROM (SELECT ");
		sql.append(" SUM(CASE WHEN stllxfcs='有' THEN 1 ELSE 0 END) as '有', ");
		sql.append(" SUM(CASE WHEN stllxfcs='无' THEN 1 ELSE 0 END) as '无',");
	...
	public List<Map> lljkTotal(String xzqhdm) {
		...
		sql.append("from hps_info_sum1  where 1=1AND jszt in(1,2) ");
		if (!xzqhdm.equals("") && xzqhdm != null) {
			String xians = xzqhdm.replace(",", "' , '");
			if (xians != null && !xians.equals("")) {
				sql.append("  AND  xian  IN (' " + xians + "  '  )");
	...
	public List<Map> lljkTotal_1(String sheng,String shi,String xian) {
		StringBuffer sql = new StringBuffer();
		sql.append("SELECT name,value FROM (SELECT ");
		sql.append(" SUM(CASE WHEN stlljkss='有' THEN 1 ELSE 0 END) as '有', ");
		sql.append(" SUM(CASE WHEN stlljkss='无' THEN 1 ELSE 0 END) as '无',");
	...
	public List<Map> gycsTotal_1(String sheng,String shi,String xian) {
		StringBuffer sql = new StringBuffer();
		sql.append("SELECT name,value FROM (SELECT ");
		sql.append(" SUM(CASE WHEN gycs='有' THEN 1 ELSE 0 END) as '有', ");
		sql.append(" SUM(CASE WHEN gycs='无' THEN 1 ELSE 0 END) as '无',");
	...
	public List<Map> zzlfTotal_1(String sheng,String shi,String xian) {
		StringBuffer sql = new StringBuffer();
		sql.append("SELECT name,value FROM (SELECT ");
		sql.append(" SUM(CASE WHEN zzflcs='有' THEN 1 ELSE 0 END) as '有', ");
		sql.append(" SUM(CASE WHEN zzflcs='无' THEN 1 ELSE 0 END) as '无',");
	...
	//开展项目环评情况，环形图
	public List<Map> kzxmhpqkTotal(String xzqhdm) {
		StringBuffer sql = new StringBuffer();
		sql.append("SELECT name,value FROM (SELECT ");
		sql.append(" SUM(CASE WHEN sftgxmhp='是' THEN 1 ELSE 0 END) as '是', ");
		sql.append(" SUM(CASE WHEN sftgxmhp='否' THEN 1 ELSE 0 END) as '否',");
		sql.append(" SUM(CASE WHEN sftgxmhp IS NULL THEN 1 ELSE 0 END) as '未知'");
		sql.append("from hps_info_sum1  where 1=1 AND jszt in(1,2) ");
		if (!xzqhdm.equals("") && xzqhdm != null) {
			String xians = xzqhdm.replace(",", "' , '");
			if (xians != null && !xians.equals("")) {
	...
	public List<Map> kzxmhpqkTotal(String sheng,String shi,String xian) {
		StringBuffer sql = new StringBuffer();
		sql.append("SELECT name,value FROM (SELECT ");
		sql.append(" SUM(CASE WHEN sftgxmhp='是' THEN 1 ELSE 0 END) as '是', ");
		sql.append(" SUM(CASE WHEN sftgxmhp='否' THEN 1 ELSE 0 END) as '否',");
	...
	//是否有环评审批
	public List<Map> sfyhpspTotal(String xzqhdm) {
		StringBuffer sql = new StringBuffer();
		sql.append("SELECT name,value FROM (SELECT ");
		sql.append(" SUM(CASE WHEN hpspsj IS  not NULL  THEN 1 ELSE 0 END) as '是', ");
		sql.append(" SUM(CASE WHEN hpspsj='' THEN 1 ELSE 0 END) as '否',");
		sql.append(" SUM(CASE WHEN  hpspsj IS NULL THEN 1 ELSE 0 END) as '未知'");
		sql.append("from hps_info_sum1  where 1=1 AND jszt in(1,2) ");
		if (!xzqhdm.equals("") && xzqhdm != null) {
			String xians = xzqhdm.replace(",", "' , '");
			if (xians != null && !xians.equals("")) {
	...
	public List<Map> sfyhpspTotal(String sheng,String shi,String xian) {
		StringBuffer sql = new StringBuffer();
		sql.append("SELECT name,value FROM (SELECT ");
		sql.append(" SUM(CASE WHEN hpspsj IS  not NULL  THEN 1 ELSE 0 END) as '是', ");
		sql.append(" SUM(CASE WHEN hpspsj='' THEN 1 ELSE 0 END) as '否',");
	...
	public List<Map> fhghTotal(String sheng,String shi,String xian) {
		StringBuffer sql = new StringBuffer();
		sql.append("SELECT name,value FROM (SELECT ");
		sql.append(" SUM(CASE WHEN sffhgh='是' THEN 1 ELSE 0 END) as '是', ");
		sql.append(" SUM(CASE WHEN sffhgh='否' THEN 1 ELSE 0 END) as '否',");
	...
	public List<Map> ghhpTotal(String sheng,String shi,String xian) {
		StringBuffer sql = new StringBuffer();
		sql.append("SELECT name,value FROM (SELECT ");
		sql.append(" SUM(CASE WHEN sffhghhp='是' THEN 1 ELSE 0 END) as '是', ");
		sql.append(" SUM(CASE WHEN sffhghhp='否' THEN 1 ELSE 0 END) as '否',");
	...
	public List<Map> hbysTotal(String sheng,String shi,String xian) {
		StringBuffer sql = new StringBuffer();
		sql.append("SELECT name,value FROM (SELECT ");
		sql.append(" SUM(CASE WHEN sftgjghbys='是' THEN 1 ELSE 0 END) as '是', ");
		sql.append(" SUM(CASE WHEN sftgjghbys='否' THEN 1 ELSE 0 END) as '否',");
	...
	public List<Map> tsgkTotal(String sheng,String shi,String xian) {
		StringBuffer sql = new StringBuffer();
		sql.append("SELECT name,value FROM (SELECT ");
		sql.append(" SUM(CASE WHEN bxsfcztsgk='是' THEN 1 ELSE 0 END) as '是', ");
		sql.append(" SUM(CASE WHEN bxsfcztsgk='否' THEN 1 ELSE 0 END) as '否',");
