...
public class SysRoleService {
	...
	public boolean setStaffNameBySysRoleId(String ids,String staffCode){
		String[] code=staffCode.split(",");
			boolean b= mSysRoleDao.updateSysRoleInfoById(ids,staffCode);
	//	mSysRoleDao.updateSysRoleInfoById(ids,staffCode);
		return true;
	}
	   ...
	   String sName="";
		...
		for(int i=0;i<list.size();i++){
			...
			if(null!=obj_){
				...
				String[] keyCode=key.split(",");
				for(int k=0;k<keyCode.length;k++) {
					SysStaff staff=mSysStaffDao.getStaffInfoByStaffCode(CommonUtil.trim(keyCode[k]));
					sName+=staff.getStaffName()+",";
				}
				if(",".equals(sName.substring(sName.length()-1))){
					sName=sName.substring(0,sName.length()-1);
				}
				list.get(i).put("STAFF_NAME", sName.toString());
			}
			sName="";
			if(null!=_obj_ ){
