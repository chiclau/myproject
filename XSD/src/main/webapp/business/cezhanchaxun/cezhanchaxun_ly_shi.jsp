...
#mainonedes_xs_jszt  thead th {
	padding: 3px 0px;
	white-space: normal;
}

#mainonedes_xs_jszt  .layui-table[lay-size=sm] td, .layui-table[lay-size=sm] th
	{
	padding: 3px 0px;
	white-space: nowrap;
}

#mainonedes_xs_kffs  thead th {
	padding: 3px 0px;
	white-space: normal !important;
}

#mainonedes_xs_kffs  .layui-table[lay-size=sm] td, .layui-table[lay-size=sm] th
	{
	padding: 3px 0px;
	white-space: nowrap;
}
			...
			<div class="row">
				<div class="col-md-6" style="margin-top: 2px;">
					<ul class="list-group list-group-one" style="position: relative">
						<li class="list-group-item" id="item-one"><i
							...
							id="title_id_js"></span>
						<button id="table-icon_xs_jszt" class="btn btn-primary table-icon"
						...
						<li class="list-group-item desplay" id="mainonedes_xs_jszt"
				...
				<div class="col-md-6" style="margin-top: -8px;">
					<ul class="list-group list-group-one" style="position: relative">
						...
						<button id="table-icon_kffs_xs" class="btn btn-primary table-icon"
								type="button">切换表格</button>
						...
						<li class="list-group-item desplay" id="mainonedes_xs_kffs"
							style="padding: 0px; position: absolute; left: 0px; top: 32px; width: 100%; z-index: 333; height: 300px;">
							<div class="layui-table-header">
								<table class="layui-table classtableone" lay-size="sm"
									style="margin: 0px;">
				...
				<div class="col-md-12">
					<ul class="list-group list-group-one" style="position: relative">
						...
						<button id="table-icon_xs_dz_fzlc"
						...
						<li class="list-group-item desplay" id="mainonedes_xs_kffs_date"
							style="padding: 0px; position: absolute; left: 0px; top: 32px; width: 100%; z-index: 333; height: 300px;">
							<div class="layui-table-header">
								<table class="layui-table classtableone" lay-size="sm"
									style="margin: 0px;">
							...
							<table class="layui-table classtablethird" lay-size="sm"
								...
								<tbody id="zj_table_xs_kffs_date">
								</tbody>
							</table>
						</li>
					</ul>
				...
				<div class="col-md-12">
					<ul class="list-group list-group-one" style="position: relative">
						...
						<button id="table-icon_xs_zj_fzlc"
								class="btn btn-primary table-icon" type="button">切换表格</button>
						...
						<li class="list-group-item desplay"
							...
							<div class="layui-table-body layui-table-main"
								...
								<table class="layui-table classtabletwo" lay-size="sm"
									...
									<tbody id="table_xs_kffs_date_zj">
									</tbody>
								</table>
							</div>
							<table class="layui-table classtablethird" lay-size="sm"
								...
								<tbody id="zj_table_xs_kffs_date_zj">
								</tbody>
							</table>
						</li>
					</ul>
	 ...
	 if(num==1){
		 fileName = address+"小水电建设状态统计" 
		  ...
		  sqlTitle = [     'name','rlz','dzz','rl1','dz1','rl2','dz2','rl3','dz3','rl4','dz4','rl5','dz5','rl6','dz6']
		 		sql = " select H.xian AS name,CAST(Round(SUM(H.zjrl)/10000,2) as DECIMAL(18,2)) AS rlz,COUNT(H.id) AS dzz,    "
		 			...
		 			+ " from hps_info_sum1 H WHERE H.shi = '"+address+"'   "
		 			+ " GROUP BY H.xian,H.shi HAVING COUNT(H.id) > 0  "
		 			+ " UNION "
		 			...
		 			+ " from hps_info_sum1 H   WHERE H.shi = '"+address+"' "
		 			+ " GROUP BY H.shi HAVING COUNT(H.id) > 0      "
	 }else if(num==2){
		 fileName = address+"小水电开发方式统计" 
			  ...
			  sqlTitle = [     'name','rlz','dzz','rl1','dz1','rl2','dz2','rl3','dz3','rl4','dz4','rl5','dz5']
		 		sql = " SELECT H.xian AS name,CAST(Round(SUM(H.zjrl)/10000,2) as DECIMAL(18,2))  AS rlz,COUNT(H.id) AS dzz,  "
		 			...
		 			+ " GROUP BY H.xian,H.shi HAVING COUNT(H.id) > 0 AND H.shi = '"+address+"'  "
		 			+ " UNION "
		 			+ " SELECT '总数' AS name,CAST(Round(SUM(H.zjrl)/10000,2) as DECIMAL(18,2)) AS rlz,COUNT(H.id) AS dzz,  "
		 			+ " SUM(CASE WHEN H.kffs = 2 then 1 else 0 end )AS dz1, "
		 			+ " SUM(CASE WHEN H.kffs = 4 then 1 else 0 end )AS dz2, "
		 			...
		 			+ " CAST(Round(SUM(CASE WHEN H.kffs = 5 then H.zjrl else 0 end )/10000,2) as DECIMAL(18,2)) AS rl5  "
		 			+ " FROM hps_info_sum1 H  "
		 			+ " GROUP BY H.shi HAVING COUNT(H.id) > 0 AND H.shi = '"+address+"' "
	 }else if(num==3){
		 fileName = address+"小水电发展历程" 
			  ...
			  sqlTitle = [     'name','rlz','dzz','rl1','dz1','rl2','dz2','rl3','dz3','rl4','dz4']
			     	sql = " SELECT A.date AS name,CAST(SUM(ISNULL(H.zjrl, 0))/10000 AS DECIMAL(18,2))  AS rlz,COUNT(H.id) AS dzz,   "
			     		...
			     		+ " FROM date_sum A LEFT JOIN hps_info_sum1 H ON A.date = DATENAME(yy, H.dysj) AND H.shi ='"+address+"' "
...
function initTable(bj){
	$.ajax({
		...
		success:function(response){
			...
			if(bj == "xs_jszt"){
				$("#table_xs_jszt").html("");
				$("#table_xs_jszt").append(tr);
			}
			if(bj == "xs_kffs"){
				$("#table_xs_kffs").html("");
				$("#table_xs_kffs").append(tr);
			}
			if(bj == "xs_kffs_date"){
				$("#table_xs_kffs_date").html("");
				$("#table_xs_kffs_date").append(tr);
			}
			if(bj == "xs_kffs_date_zj"){
				$("#table_xs_kffs_date_zj").html("");
				$("#table_xs_kffs_date_zj").append(tr);
			}
			if(bj != "xs_kffs_date" && bj != "xs_kffs_date_zj"){
				var zj_tr = "<tr>"
					+"<td colspan = '2'style='display:inline-block;width: 135px;'>总计</td>"
			...
			if(bj == "xs_jszt"){
				$("#zj_table_xs_jszt").html("");
				$("#zj_table_xs_jszt").append(zj_tr);
			}
			if(bj == "xs_kffs"){
				$("#zj_table_xs_kffs").html("");
				$("#zj_table_xs_kffs").append(zj_tr);
			}
			if(bj == "xs_kffs_date"){
				$("#zj_table_xs_kffs_date").html("");
				$("#zj_table_xs_kffs_date").append(zj_tr);
			}
			if(bj == "xs_kffs_date_zj"){
				$("#zj_table_xs_kffs_date_zj").html("");
				$("#zj_table_xs_kffs_date_zj").append(zj_tr);
			}
		}
	})
}
function initKffs(bj,address){
    ...
    if(bj == 1 || bj == 2){
		...
		success:function(response){
			...
			myChart.on('click', function (params) {
				...
				layer.open({
					  ...
					  icon: 6,
					  content: basePath+'business/cezhanchaxun/modalFrame.jsp?name='+name+"&bj="+7+"&address="+address,
				});
			});
		     // 使用刚指定的配置项和数据显示图表。
     ...
     //点击切换图标文字和显示隐藏table
     $('#table-icon_kffs_xs').click(function(){
    	 if( $('#mainonedes_xs_kffs').hasClass('desplay')){
    		 $('#table-icon_kffs_xs').text('切换图表');
    		 $('#mainonedes_xs_kffs').removeClass('desplay');
    		 ...
    		 initTable("xs_kffs")
    		  ...
    		  $('#mainonedes_xs_kffs').addClass('desplay');
    		  $('#table-icon_kffs_xs').text('切换表格');
...
function initJszt(bj,address){
 		...
 		success:function(response){
			...
			myChart.on('click', function (params) {
				...
				layer.open({
					  ...
					  skin: 'layui-layer-molv' ,
					  icon: 6,
					  content: basePath+'business/cezhanchaxun/modalFrame.jsp?name='+name+"&bj="+8+"&address="+address,
				});
			});
				     // 使用刚指定的配置项和数据显示图表。
				     myChart.setOption(option);
     ...
     //点击切换图标文字和显示隐藏table
     $('#table-icon_xs_jszt').click(function(){
    	 if( $('#mainonedes_xs_jszt').hasClass('desplay')){
    		 $('#table-icon_xs_jszt').text('切换图表');
    		 $('#mainonedes_xs_jszt').removeClass('desplay');
    		 ...
    		 initTable("xs_jszt")
    		  ...
    		  $('#mainonedes_xs_jszt').addClass('desplay');
    		  $('#table-icon_xs_jszt').text('切换表格');
    		  $('#table-iconone').addClass('desplay');
    	 }
     })
}
...
function initZjrl(bj,address){
			...
			option = {
			 ...
			 myChart.on('click', function (params) {
					...
					layer.open({
						  ...
						  skin: 'layui-layer-molv' ,
						  icon: 6,
						  content: basePath+'business/cezhanchaxun/modalFrame_fzlc.jsp?name='+name+"&bj="+4+"&address="+address,
					});
				});
			    // 使用刚指定的配置项和数据显示图表。
			    myChart.setOption(option);
    ...
    //点击切换图标文字和显示隐藏table
    $('#table-icon_xs_zj_fzlc').click(function(){
   	 if( $('#mainonedes_xs_kffs_date_zj').hasClass('desplay')){
   		 $('#table-icon_xs_zj_fzlc').text('切换图表');
   		 $('#mainonedes_xs_kffs_date_zj').removeClass('desplay');
   		 if($('#qg_zj_fzlc-iconone').hasClass('desplay')){
			 $('#qg_zj_fzlc-iconone').removeClass('desplay');
		 }else{
			 $('#table-iconone').addClass('desplay');
		 }
   		 initTable("xs_kffs_date_zj")
   	 }
   	  else{
   		  $('#mainonedes_xs_kffs_date_zj').addClass('desplay');
   		  $('#table-icon_xs_zj_fzlc').text('切换表格');
...
function initDzsl(bj,address){
    ...
    //点击切换图标文字和显示隐藏table
    $('#table-icon_xs_dz_fzlc').click(function(){
   	 if( $('#mainonedes_xs_kffs_date').hasClass('desplay')){
   		 $('#table-icon_xs_dz_fzlc').text('切换图表');
   		 $('#mainonedes_xs_kffs_date').removeClass('desplay');
   		 if($('#fzlc-iconone').hasClass('desplay')){
			 $('#fzlc-iconone').removeClass('desplay');
		 }else{
			 $('#table-iconone').addClass('desplay');
		 }
   		 initTable("xs_kffs_date")
   	 }
   	  else{
   		  $('#mainonedes_xs_kffs_date').addClass('desplay');
   		  $('#table-icon_xs_dz_fzlc').text('切换表格');
