...
function step5ZfglzCombox(){
	var bd=beginDate.substring(0,10)+" 00:00:00";
	var ed=endDate.substring(0,10);
	var edate=addDate(ed)+" 23:59:59";
	$("#step5_emstcd").combox({
		...
		select:function(item){
			loadStep5EdataValues(item.STCD,bd,edate);
		}
	});
}
function loadStep5EdataValues(stcd,bd,ed){
	$.ajax({
		url:basePath+"chanliu/chanliu!queryEdata.action?start="+bd+"&end="+ed+"&stcd="+stcd+"&INTERVAL="+interval,
		type:"post",
		dataType:"json",
		success:function(response){
			if(response.reflag==1||response.reflag=="1"){
				...
				$("#stept5_table_tbdody tr").each(function(index,tr){
					...
					dt=(new Date(dt+" "+_zfltime).getTime()>new Date(tm).getTime())?dt:addDate(dt,1);
					var key=dt.replace(/-/g, "");
					key="key"+key;
					$(tds[parseInt(num)+3]).find("input").val(((eobject[key]!=null && eobject[key]!="")?eobject[key]:0));
				});
			}else{
				layer.msg(response.message);
			}
	...
	var tr=$(".removeTr");
	 ...
	 for(var i=0;i<tm.length;i++){ //这个循环组成新的数组
		...
		if(_faparamsValue){
			...
			coefdata.KG=_faparamsValue.EX;
			if(_faparamsValue["KG"]==null){
				layer.msg("地下水出流系数KG值未配置!");
				return ;
			}
			coefdata.KI=_faparamsValue.KG;
