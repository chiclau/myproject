...
public class FsdrDao extends HibernateBaseDao<Fsdr>{
	...
	public PageResults getFsdrListData(FsdrFormBean mFsdrFormBean){
		...
		sql.append("SELECT B.STCD AS STCD_1,C.STCD AS STCD_2,FSDR.UPSTCD,FSDR.DWSTCD,");
		sql.append("FSDR.RCHLEN,FSDR.SFTQ,FSDR.QMGN,FSDR.MNTRTM,FSDR.MXTRTM,FSDR.AVTRTM,");
		sql.append("FSDR.MODITIME,B.STNM AS STNM_1,C.STNM AS STNM_2 FROM ST_FSDR_B FSDR ");
		sql.append("INNER JOIN ST_STBPRP_B B ON FSDR.UPSTCD=B.STCD ");
		sql.append("INNER JOIN ST_STBPRP_B C ON FSDR.UPSTCD=C.STCD WHERE 1=1 ");
		if(null!=str && !"".equals(str)){
			sql.append(str);
		}
		sql.append(" ORDER BY FSDR.UPSTCD");
		return this.findPageByFetchedSql(sql.toString(), null
				,mFsdrFormBean.getPageBean().getOffset()
	...
	public PageResults getFsdrListData_export(FsdrFormBean mFsdrFormBean){
		StringBuilder sql=new StringBuilder();
		String str=spliceStrFsdr(mFsdrFormBean);
		sql.append("SELECT UPSTCD,DWSTCD,RCHLEN,SFTQ,QMGN,MNTRTM,MXTRTM,AVTRTM FROM ST_FSDR_B WHERE 1=1 ");
		if(null!=str && !"".equals(str)){
			sql.append(str);
		}
