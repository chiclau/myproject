...
public class StbprpControl {
	...
	@Optlog(menuflag="stbprpListMoHu", opttype = "getStbprpMes") 
	public RetMessage getStbprpMesMoHu(StbprpFormBean mStbprpFormBean,PageResults mPageResults){
		RetMessage mRetMessage=new RetMessage();
		try{
			BeanUtils.copyProperties(mPageResults,mStbprpService.getStbprpListMoHu(mStbprpFormBean));
			mRetMessage.setRetflag(RetMessage.RETFLAG_SUCCESS);
			mRetMessage.setMessage("查询数据成功！");
		}catch (Exception e) {
			mRetMessage.setRetflag(RetMessage.RETFLAG_ERROR);
