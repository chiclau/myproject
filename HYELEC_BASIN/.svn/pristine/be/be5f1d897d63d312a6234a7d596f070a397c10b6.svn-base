...
public class RvsectDao extends HibernateBaseDao<Rvsect>{
	...
	public PageResults getRvsectListData(RvsectFormBean mRvsectFormBean){
		...
		if(mRvsectFormBean.getmRvsectInfoBean().getStcd()!=null&&!"".equals(mRvsectFormBean.getmRvsectInfoBean().getStcd().trim())){
			...
			if(null!=str && !"".equals(str)){
				sql.append(str);
				sql.append(" ORDER BY convert(int,[VTNO])");
			}else{
				sql.append(" ORDER BY rvsect.STCD,rvsect.MSTM");
			}
		}else{
			sql.append(" SELECT rvsect.STCD,rvsect.MSTM,b.STNM FROM ST_RVSECT_B rvsect LEFT JOIN ST_STBPRP_B b ON rvsect.STCD = b.STCD WHERE 1 = 1  ");
			if(mRvsectFormBean.getSearchName()!=null&&!"".equals(mRvsectFormBean.getSearchName().trim())){
				sql.append(" AND (b.STNM  LIKE '%"+CommonUtil.trim(mRvsectFormBean.getSearchName())+"%') ");
			}
			sql.append(" GROUP BY rvsect.STCD,rvsect.MSTM,b.STNM ORDER BY rvsect.STCD ");
		}
	...
	public List<Map> getRvsectInfoByParams(RvsectFormBean mRvsectFormBean){
		StringBuilder sql=new StringBuilder();
		sql.append("SELECT VTNO FROM ST_RVSECT_B ");
		sql.append("WHERE STCD='"+mRvsectFormBean.getmRvsectInfoBean().getStcd()+"' ");
		return this.createSQLQuerybyMap(sql.toString());
	}
	...
	private String spliceStrRvsect(RvsectFormBean mRvsectFormBean){
		StringBuilder sql=new StringBuilder();
		if(null!=mRvsectFormBean&&!"".equals(mRvsectFormBean.getmRvsectInfoBean().getVtno().trim())){
			sql.append(" AND rvsect.VTNO = '"+CommonUtil.trim(mRvsectFormBean.getmRvsectInfoBean().getVtno())+"'");
		}
	...
	public void delptno(RvsectFormBean mRvsectFormBean, String str) {
		StringBuilder sql=new StringBuilder();
		sql.append("DELETE FROM ST_RVSECT_B WHERE STCD='"+mRvsectFormBean.getmRvsectInfoBean().getStcd()+"' AND VTNO='"+str+"'");
		this.exectueSQL(sql.toString());
	}
}
