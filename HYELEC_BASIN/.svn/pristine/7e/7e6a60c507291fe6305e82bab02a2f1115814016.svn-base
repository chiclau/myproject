package com.lyht.business.modelManage.formbean;

import java.io.Serializable;

import com.lyht.base.hibernate.common.PageResults;
import com.lyht.business.modelManage.bean.ModelParamenter;

/**
 *作者： 刘魁
 *脚本日期:2018年5月7日 21:41:11
 *说明:  模型参数值FormBean
*/
@SuppressWarnings("rawtypes")
public class ModelParaFormBean implements Serializable{

	private static final long serialVersionUID = 1L;
	private String searchName; //用于快速模糊匹配关键字
    private String ids; //用于批量选中多ids，以","分隔 如：1,2,3,4
    private ModelParamenter modelParFormBean=new ModelParamenter();
    private PageResults pageBean=new PageResults();
	public String getSearchName() {
		return searchName;
	}
	public void setSearchName(String searchName) {
		this.searchName = searchName;
	}
	public String getIds() {
		return ids;
	}
	public void setIds(String ids) {
		this.ids = ids;
	}
	public ModelParamenter getModelParFormBean() {
		return modelParFormBean;
	}
	public void setModelParFormBean(ModelParamenter modelParFormBean) {
		this.modelParFormBean = modelParFormBean;
	}
	public PageResults getPageBean() {
		return pageBean;
	}
	public void setPageBean(PageResults pageBean) {
		this.pageBean = pageBean;
	}
    
    
    
}
