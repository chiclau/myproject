			...
			if(lnnm != "" && stcd!=""){
				...
				common_ajax(null,url, function(response) {
					...
					for(var i =0;i<response.mZqrlFormBean.length;i++){
						...
						var tr = "<tr>"+
						"<td style='text-align: left'><input style='margin-left: 15px;width: 90px' class='a' readonly=true id='ptno_zqrl_x' value="+response.mZqrlFormBean[i].PTNO+" name='ptno'></td>"+
						"<td style='text-align: left'><input style='margin-left: 15px;width: 90px' class='a' id='z_zqrl_x' value="+response.mZqrlFormBean[i].Z+" name='z'></td>"+
						"<td style='text-align: left'><input style='margin-left: 15px;width: 90px' class='a' id='q_zqrl_x' value="+response.mZqrlFormBean[i].Q+" name='q'></td>"+
					...
					for(var i=0;i<response.mZqrlFormBean.length;i++){  
		               var date = formatDate(response.mZqrlFormBean[i].MODITIME,'yyyy-MM-dd');
		               times.push(date);
		            }
		            for(var i=0;i<response.mZqrlFormBean.length;i++){  
		           	 waterLevel.push(response.mZqrlFormBean[i].Z); 
		            }
		            for(var i=0;i<response.mZqrlFormBean.length;i++){  
		           	 flow.push(response.mZqrlFormBean[i].Q);  
		            }  
		            //统计图表初始化
...
//					            saveAsImage : {show: true}   
	 ...
	 // 重置窗体
		...
		function x_xzcs(){
			var table = $("#query_table_waterLevelFlow_x");
			var tr = "<tr>"+
				"<td style='text-align: left'><input style='margin-left: 15px;width: 90px' readonly=true; value='"+(++len)+"' name='ptno' id='ptno_zqrl_x'></td>"+
