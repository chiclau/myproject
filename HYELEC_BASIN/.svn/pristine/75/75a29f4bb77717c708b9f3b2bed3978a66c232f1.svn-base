package com.lyht.business.analysisCalculation.formbean;

import java.io.Serializable;

import com.lyht.base.hibernate.common.PageResults;
import com.lyht.business.analysisCalculation.bean.ModelProgValue;
/**
 *作者： 刘魁
 *脚本日期:2018年5月9日 15:41:11
 *说明:  方案参数值FormBean
*/
public class ModelProgValFromBean implements Serializable {
	private static final long serialVersionUID = 1L;
	private String searchName; //用于快速模糊匹配关键字
    private String ids; //用于批量选中多ids，以","分隔 如：1,2,3,4
    private ModelProgValue ModelProgValFromBean=new ModelProgValue();
    private PageResults pageBean=new PageResults();
	public String getSearchName() {
		return searchName;
	}
	public void setSearchName(String searchName) {
		this.searchName = searchName;
	}
	public String getIds() {
		return ids;
	}
	public void setIds(String ids) {
		this.ids = ids;
	}
	public ModelProgValue getModelProgValFromBean() {
		return ModelProgValFromBean;
	}
	public void setModelProgValFromBean(ModelProgValue modelProgValFromBean) {
		ModelProgValFromBean = modelProgValFromBean;
	}
	public PageResults getPageBean() {
		return pageBean;
	}
	public void setPageBean(PageResults pageBean) {
		this.pageBean = pageBean;
	}
    
    
}
