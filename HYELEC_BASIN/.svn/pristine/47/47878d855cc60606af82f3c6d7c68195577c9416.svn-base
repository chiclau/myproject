...
public class TsqxDao extends HibernateBaseDao<Tsqx>{
	...
	public PageResults getTsqxListData(TsqxFormBean mTsqxFormBean){
		...
		if(mTsqxFormBean.getmTsqxInfoBean().getStcd()!=null&&!"".equals(mTsqxFormBean.getmTsqxInfoBean().getStcd().trim())){
			sql.append(" SELECT a.STCD,a.USERNAME,a.QM,a.Q,a.T,b.STNM FROM ST_TSQX_B a LEFT JOIN ST_STBPRP_B b ON a.STCD = b.STCD WHERE 1 = 1 ");
			...
			sql.append(" ORDER BY a.QM");
		}else{
			sql.append(" SELECT a.STCD,a.USERNAME,b.STNM FROM ST_TSQX_B a LEFT JOIN ST_STBPRP_B b ON a.STCD = b.STCD WHERE 1 = 1   ");
