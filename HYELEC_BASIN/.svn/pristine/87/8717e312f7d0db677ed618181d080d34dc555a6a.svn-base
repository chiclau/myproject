(function($) {
	...
	$.DiurnalEvaporation_Query = function(option) {
		...
		var opt_control={
			...
			,query_startTime_dayev			:$('#startTime_dayev')	//页面查询起始时间
			,query_endTime_dayev			:$('#endTime_dayev')	//页面查询结束时间
			,query_stcd_dayev				:$('#stcd_dayev')	//页面查询根据测站名称
			...
			,btn_stcd_dayev					:$('#stcd_dayev_list')		//测站下拉框
		}
		...
		this.InitData=function(opt){
			...
			function queryParams(params) {  // 配置参数
				...
				var opt_parms={
					...
					,"mDayevFormBean.startTime"				:opt_control.query_startTime_dayev.val()//页面查询起始时间
					,"mDayevFormBean.endTime"				:opt_control.query_endTime_dayev.val()//页面查询结束时间
					,"mDayevFormBean.stcd"					:opt_control.btn_stcd_dayev.val()//页面查询根据测站名称
				};
		...
		this.InitSelect = function (){
			...
			Load_select_stcd_dayev_list();
		}
		
		function Load_select_stcd_dayev(){
			opt_control.query_stcd_dayev.empty();
			var url = opt_all.url_upstcd_Stbprp;
			opt_control.query_stcd_dayev.append("<option value=''>请选择测站名称</option>");
			common_ajax(null,url, function(data) {
				console.log(JSON.stringify(data));
				$.each(data.rows, function(i) {
					opt_control.query_stcd_dayev.append(
						'<option value=' + data.rows[i].STCD + '>'+ data.rows[i].STNM + '</option>');
				});
			});
		}
		function Load_select_stcd_dayev_list(){
