			...
			if(stcd != "" ){
				...
				common_ajax(null,url, function(response) {
					...
					for(var i =0;i<response.mRvsectFormBean.length;i++){
						...
						"<td style='text-align: left'><input style='margin-left: 15px;width: 90px' class='a' id='di_rvsect_x' name='di' value="+response.mRvsectFormBean[i].DI+"></td>"+
						"<td style='text-align: left'><input style='margin-left: 15px;width: 90px' class='a' id='zb_rvsect_x' name='zb' value="+response.mRvsectFormBean[i].ZB+"></td>"+
					...
					for(var i=0;i<response.mRvsectFormBean.length;i++){  
			               var date = formatDate(response.mRvsectFormBean[i].MODITIME,'yyyy-MM-dd');
			               times.push(date);
			            }
			            for(var i=0;i<response.mRvsectFormBean.length;i++){  
			            	startingDistance.push(response.mRvsectFormBean[i].DI); 
			            }
			            for(var i=0;i<response.mRvsectFormBean.length;i++){  
			            	riverBottomHeight.push(response.mRvsectFormBean[i].ZB);  
			            }  
			            //统计图表初始化
...
//						            saveAsImage : {show: true}   
		...
		function x_xzcs(){
			var table = $("#query_table_sectionTest_x");
			var tr = "<tr>"+
				"<td style='text-align: left'><input style='margin-left: 15px;width: 90px' readonly='readonly' class='a' name='vtno' id='vtno_rvsect_x' value='"+(++len)+"' ></td>"+
				"<td style='text-align: left'><input style='margin-left: 15px;width: 90px' class='a' name='di' id='di_rvsect_x' ></td>"+
				"<td style='text-align: left'><input style='margin-left: 15px;width: 90px' class='a' name='zb' id='zb_rvsect_x' ></td>"+
