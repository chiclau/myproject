...
public class ModelInfoService {
	 ...
	 * */
	@Transactional(propagation=Propagation.REQUIRED)
	public ModelInfo create(ModelInfo modelInfo,ModelParamenter mp) {
		...
		if(paraName.length>0) {
			for(int i=0;i<paraName.length;i++){
				ModelParamenter mp1=new ModelParamenter();
				mp1.setModelCode(modelCode);
				String paraCode=Randomizer.nextNumber("mxcs", 4);
				mp1.setParaCode(paraCode.trim());
				mp1.setParaName(paraName[i].trim());
				mp1.setParaSymbol(paraSymbol[i].trim());
				mp1.setParaType(0);
				mp1.setParaMax(paraMax[i].trim());
				mp1.setParaMin(paraMin[i].trim());
				mPDao.saveModelParamenter(mp1);  //保存值
			}
		}
		return modelInfo;
	}
	 ...
	 * */
	@Transactional(propagation=Propagation.REQUIRED)
	public ModelInfo update(ModelInfo modelInfo,ModelParamenter mp){
