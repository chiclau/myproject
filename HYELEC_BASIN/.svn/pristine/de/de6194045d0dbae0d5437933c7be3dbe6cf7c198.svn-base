...
var rrffDataQuery=new $.RrffData_Query
...
$(function(){
	...
	rrffDataQuery.InitData(opt);
	rrffDataQuery.InitAddEditDel(opt);
	rrffDataQuery.InitSelect();
	// 窗体变化时，调整组件的大小
...
function _AutoSize(){
	...
	$("#query_table_rrff").bootstrapTable("resetView",{"height":comm_getHeight()-80});  
...
function FMT_Num6(value,row,index){
	var pageNumber = $("#query_table_rrff").bootstrapTable('getOptions').pageNumber;
	var pageSize   = $("#query_table_rrff").bootstrapTable('getOptions').pageSize;
	return (pageNumber-1) * pageSize+index+1;
}
...
//降水径流相关图
function FMT_handle6(value,row){
	var html="";
	html+="<a href='#' onclick='javascript:rrffDataQuery.edit(\""+row.STCD + "\",\""+row.USERNAME + "\")'>" +
	"<button class='btn btn-xs btn-primary'>" +
	"<div class='visible-md visible-lg'><i class='icon icon-pencil'></i>&nbsp;修改</div>" +
	"<div class='visible-xs visible-sm'><i class='icon icon-pencil'></i></div>" +
	"</button></a>" +
	"&nbsp;&nbsp;" +
	"<a href='#' onclick='javascript:rrffDataQuery.del(\""+row.STCD + "\",\""+row.USERNAME + "\")'>" +
	"<button class='btn btn-xs btn-danger btn_del_color'>" +
	"<div class='visible-md visible-lg'><i class='icon icon-trash'></i>&nbsp;删除</div>" +
	"<div class='visible-xs visible-sm'><i class='icon icon-trash'></i></div>" +
	"</button></a>" +
