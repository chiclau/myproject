	...
	public boolean setStaffNameBySysGroupId(String []ids,String []staffCode){
		...
		for(int i=0;i<staffCode.length;i++){
			for(int j=0;j<ids.length;j++){
				List<Map> list=mSysGroupDao.getSysGroupListById(Integer.parseInt(ids[0]));
				if(list.size()>0){
					Object obj=list.get(0).get("STAFF_CODE");
					if(null!=obj){
						String staffCode_=obj.toString();  //数据库中的值
						if(null==staffCode_ || "".equals(staffCode_)){
							flag=mSysGroupDao.updateSysGroupStaffCodeBySysGroupId(ids[0],staffCode[i]);
						}else{
							...
							flag=mSysGroupDao.updateSysGroupStaffCodeBySysGroupId(ids[0],str);
						}
					}
				}
			}
